...
class service_profile {
	...
	function create_service_profile() {
		try {
			...
			$result = $conn->db->query("INSERT INTO SERVICE_PROFILE (NAME, PORTS, SERVICE_PROFILE_ID) VALUES ('$this->name', '$this->ports', '$this->service_profile_id')");
	...
	function edit_service_profile() {
		try {
			$conn = db_connect::getInstance();
			$conn->db->query("UPDATE SERVICE_PROFILE SET NAME = '$this->name', PORTS = '$this->ports', SERVICE_PROFILE_ID = '$this->service_profile_id' where ID = '$this->id'");
		} catch (PDOException $e) {
			$error = "Connection Failed:" . $e->getMessage() . "\n";
			return $error;
		}
	...
	function build_table_service_profile() {
		try {
			$conn = db_connect::getInstance();
			$result = $conn->db->query("SELECT SERVICE_PROFILE.ID, SERVICE_PROFILE.NAME, SERVICE_PROFILE.PORTS, SERVICE_PROFILE.SERVICE_PROFILE_ID from SERVICE_PROFILE");
		} catch (PDOException $e) {
			$error =  "Connection Failed:" . $e->getMessage() . "\n";
			return $error;
		}
	...
	function get_data_service_profile() {
		try {
			$conn = db_connect::getInstance();
			$result = $conn->db->query("SELECT NAME, PORTS, SERVICE_PROFILE_ID from SERVICE_PROFILE where ID='$this->id'");
		} catch (PDOException $e) {
